...
在类似 electron 等应用中，用户不是直接通过 node 启动，此时可以通过 NODE_OPTIONS 环境变量设置代理,

```json
// cross-env 来兼容不同系统的环境变量配置
"scripts": {
   "dev": "xxx",
   "dev:proxy": "cross-env NODE_OPTIONS=\"--require global-agent/bootstrap\" npm run dev"
}
```
